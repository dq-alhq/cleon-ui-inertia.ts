...
import { GridList, GridListItem, type GridListItemProps, Header, Link, type SectionProps } from 'react-aria-components'
...
const SidebarBody = ({ children, className, ...props }: React.HTMLProps<HTMLDivElement>) => (
    <nav className={cn('flex flex-col overflow-y-auto p-4 space-y-2 [&>section+section]:mt-8', className)} {...props}>
        {children}
    </nav>
)
...
const SidebarSection = <T extends object>({ className, ...props }: SidebarSectionProps<T>) => {
    return (
                ...
                (props) =>
                    ...
                    (props.children &&
                        React.Children.toArray(props.children).some(
                            (descendant) => (descendant as React.ReactElement).props.active
                        ))
            )
        ...
        return (
            <GridListItem textValue={props.textValue} {...props}>
                <Accordion defaultExpandedKeys={isExpanded ? [0] : []} hideBorder={true}>
                    <Accordion.Item currentId={0} className='border-0 focus:outline-none pb-0.5' key={0}>
                        <Accordion.Trigger
                            className={cn(
                        ...
                        <Accordion.Content className='p-0 [&_.acrt]:ml-4 [&_.subitem]:ml-4 [&_.acrt+&_.subitem]:ml-8 [&_.subitem]:mb-1'>
...
const SidebarHeader = ({ className, ...props }: React.HtmlHTMLAttributes<HTMLDivElement>) => (
    <div className={cn('flex flex-col border-b p-4 [&>section+section]:mt-2.5', className)} {...props} />
)

const SidebarFooter = ({ className, ...props }: React.HtmlHTMLAttributes<HTMLDivElement>) => (
    <div className={cn('flex flex-col mt-auto border-t p-4 [&>section+section]:mt-2.5', className)} {...props} />
)
