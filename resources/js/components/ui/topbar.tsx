...
import { Header, ListBox, ListBoxItem, type ListBoxItemProps, type ListBoxProps } from 'react-aria-components'
...
const Topbar = ({ children, className, ...props }: TopbarProps) => {
    return (
        <Header {...props} className={cn('sticky top-0 z-30 border-b bg-background/40 backdrop-blur-sm', className)}>
            <div className='mx-auto max-w-screen-2xl py-2 px-4'>
                <div className='flex items-center w-full lg:justify-normal justify-between'>{children}</div>
            </div>
        </Header>
    )
}
...
const Navigations = <T extends object>({ children, isResponsive = true, className, ...props }: NavigationsProps<T>) => {
