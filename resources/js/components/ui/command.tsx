...
const { command, empty, section, list, item, closeButton, modal, input, modalOverlay, kbdKeyboard, description } =
    commandStyles()
    ...
    return (
        <CommandPrimitive.List className={list({ className })} {...props}>
            {messageOnEmpty !== false && (
                <CommandEmpty>{typeof messageOnEmpty === 'string' ? messageOnEmpty : 'No results found.'}</CommandEmpty>
            )}
            {props.children}
